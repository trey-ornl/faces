...
export CPPFLAGS="-DMAXTHREADS=${MAXTHREADS} -DFUSE_INNER"
build v-26-messages-device-sync -inner

export CPPFLAGS="-DMAXTHREADS=${MAXTHREADS} -DFUSE_INNER"
build v-26-messages-events -inner

export CPPLFAGS="-DMAXTHREADS=${MAXTHREADS} -DFUSE_INNER"
build v-26-messages-pipelined-recvs -inner

export CPPFLAGS="-DMAXTHREADS=${MAXTHREADS} -DFUSE_INNER"
build v-26-messages-single-stream -inner

export CPPFLAGS="-DMAXTHREADS=${MAXTHREADS} -DFUSE_INNER"
build v-26-messages-single-stream-event -inner
