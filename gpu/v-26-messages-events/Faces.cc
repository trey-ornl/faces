  ...
  zfs_(n,n,mx,my,2)
{
  ...
  for (int i = 0; i < nEvents_; i++) CHECK(gpuEventCreate(event_+i));
...
Faces::~Faces()
{
  for (int i = 0; i < nEvents_; i++) CHECK(gpuEventDestroy(event_[i]));
...
void Faces::share(Double6D &u, const bool compute)
{
  ...
  // send in computed order

  CHECK(gpuEventSynchronize(event_[0]));
  ...
  CHECK(gpuEventSynchronize(event_[1]));
  ...
  CHECK(gpuEventSynchronize(event_[2]));
